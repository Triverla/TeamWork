...
const generateId = require('../helpers/identity');
const { validatePost, validateEdit } = require('../models/validations/article.validate');
...
class ArticleController {
  static async createSingleArticle(req, res) {
    ...
    await db.query(
      `INSERT INTO articles (articleId, title, article, created, categoryId, author) 
        VALUES ($1, $2, $3, $4, $5, $6)`, [articleId, title, article, created, categoryId, author],
    );
  ...
  static async deleteSingleArticle(req, res) {
    ...
    const owner = await db.query(`SELECT * FROM articles WHERE articleId = ${articleId}`);
    if (owner.rowCount === 0) return res.status(404).json({ message: 'Article Not Found' });
    if (owner.rows[0].author !== req.user.email) {
      return res.status(403).json({
        status: 'error',
  ...
  static async getAllArticles(req, res) {
    const articles = await db.query('SELECT * FROM articles ORDER BY created DESC');
